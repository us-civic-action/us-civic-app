...
import { checkSavedBills } from '@/app/actions';
...
export default function Dashboard({ hearings, amendments = [], initialViewMode = 'hearings', stateCode }: { hearings: Hearing[], amendments?: AmendedBill[], initialViewMode?: 'hearings' | 'amendments' | 'saved', stateCode: string }) {
    ...
    const [updatedBillIds, setUpdatedBillIds] = useState<Set<number>>(new Set());
    ...
    return (
        <div className="max-w-4xl mx-auto px-6 mt-8 -translate-y-4">
            ...
            <div className="flex justify-center mb-8">
                <div className="bg-granite-50 p-1 rounded-xl shadow-sm border border-granite-200 inline-flex items-center">
                    ...
                    >
                        ...
                        {updatedBillIds.size > 0 && (
                            <span className="ml-1 flex h-3 w-3 relative">
            ...
            {/* Notification Banner */}
            {
                updatedBillIds.size > 0 && viewMode === 'saved' && (
                    <div className="mb-6 bg-indigo-50 dark:bg-indigo-900/30 border border-indigo-100 dark:border-indigo-800 p-4 rounded-xl flex items-center gap-3 shadow-sm">
                        <span className="text-2xl">🔔</span>
                        <div>
                            <h3 className="font-bold text-indigo-900 dark:text-indigo-200">Updates Available!</h3>
                            <p className="text-sm text-indigo-700 dark:text-indigo-300">
                                {updatedBillIds.size} of your saved bills have new activity.
                            </p>
                        </div>
                        <button
                            onClick={() => {
                                // Scroll to upgrades or filter? For now just clear.
                                // Actually, let's keep them highlighted.
                            }}
                            className="ml-auto text-xs font-bold text-indigo-600 hover:text-indigo-800 px-3 py-1"
                        >
                            Dismiss
                        </button>
                    </div>
                            ...
                            <div className="flex items-center gap-2">
                                <button
                                    onClick={async () => {
                                        ...
                                        const updates = await checkSavedBills(savedIds, stateCode);

                                        // Compare updates with local storage Last Checked? 
                                        // For now, we just show if there is *any* activity that differs from what we knew?
                                        // Actually, simpler: We highlight ANY bill that moved recently (e.g. last 7 days)
                                        // But checkSavedBills returns basic info. 

                                        // Let's just highlight bills where last_action_date is recent?
                                        // Or better, let's just highlight them all if they are in the returned set (which matches the masterlist)
                                        // The MasterList implies they are "active" in some sense, or we can filter by date.

                                        // Logic: Highlight if date > 7 days ago?
                                        const recent = updates.filter(u => {
                                            const d = new Date(u.last_action_date as string);
                                            const now = new Date();
                                            const diff = (now.getTime() - d.getTime()) / (1000 * 3600 * 24);
                                            return diff < 14; // Highlight activity in last 2 weeks
                                        });

                                        setUpdatedBillIds(new Set(recent.map(r => r.bill_id)));
                                        setCheckingUpdates(false);
                                    }}
                                    disabled={checkingUpdates}
                                    className="text-xs font-bold bg-granite-100 dark:bg-slate-800 hover:bg-granite-200 text-granite-700 dark:text-slate-300 px-3 py-1.5 rounded-lg transition-colors flex items-center gap-2"
                                >
                                    {checkingUpdates ? (
                                        <>
                                    ...
                                    {savedHearings.length > 0 && (
                                        <div className="space-y-4">
                                            ...
                                            {savedHearings.map(hearing => {
                                                ...
                                                const smartPills = hearing.smart_pills || [];
                                                const hasUpdate = updatedBillIds.has(hearing.bill_id!);
                                                return (
                                                    <div key={hearing.hearing_id} className={`bg-granite-50 rounded-xl shadow-sm border border-l-4 ${hasUpdate ? 'border-l-indigo-500 ring-2 ring-indigo-500/20' : 'border-l-nh-green-500'} border-granite-100 p-6 flex items-start gap-4 transition-all`}>
                                                        <div className="flex-1">
                                    ...
                                    {savedAmendments.length > 0 && (
                                        <div className="space-y-4">
                                            ...
                                            {savedAmendments.map(bill => {
                                                const hasUpdate = updatedBillIds.has(bill.bill_id);
                                                return (
                                                    <div key={bill.bill_id} className={`bg-granite-50 rounded-xl shadow-sm border border-l-4 ${hasUpdate ? 'border-l-indigo-500 ring-2 ring-indigo-500/20' : 'border-l-orange-500'} border-granite-100 p-6`}>
                                                        <div className="flex items-center gap-2 mb-2">
                                                            <span className="text-sm font-bold text-freedom-blue-700 dark:text-blue-300 bg-blue-50 dark:bg-blue-900/30 px-2 py-0.5 rounded border border-blue-100 dark:border-blue-800">
                                                                {bill.bill_number}
                                                            </span>
