...
export default function Dashboard({ hearings, amendments = [], initialViewMode = 'hearings' }: { hearings: Hearing[], amendments?: AmendedBill[], initialViewMode?: 'hearings' | 'amendments' | 'saved' }) {
    ...
    // Saved Bills Hook
    const { savedIds, toggleSave, isSaved, mounted } = useSavedBills();
    ...
    const BookmarkButton = ({ billId }: { billId?: number }) => {
        ...
        return (
            <button
                ...
                className={`ml-2 p-1.5 rounded-full transition-all ${active
                    ? 'text-yellow-500 bg-yellow-50 dark:bg-yellow-900/30 dark:text-yellow-400 hover:bg-yellow-100'
                    : 'text-granite-300 dark:text-slate-600 hover:text-yellow-500 hover:bg-yellow-50 dark:hover:bg-slate-700'}`}
    ...
    return (
        <div className="max-w-4xl mx-auto px-6 mt-8 -translate-y-12">
            {/* View Mode Tabs */}
                        ...
                        {filteredHearings.map((hearing) => {
                            ...
                            return (
                                <div
                                    ...
                                    className="bg-granite-50 rounded-xl shadow-sm border border-granite-100 p-6 hover:shadow-md transition-shadow relative overflow-hidden"
                                >
                                    {/* AI Ribbon */}
                                    {hearing.ai_summary && (
                                        <div className="absolute top-0 right-0 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 text-xs font-bold px-3 py-1 rounded-bl-lg border-b border-l border-indigo-100 dark:border-indigo-800 flex items-center gap-1">
                                    ...
                                    <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
                                        <div className="flex-1">
                                            ...
                                            <div className="flex flex-wrap items-center gap-2 mb-3 pt-4 md:pt-0">
                                                ...
                                                {hearing.bill_status && (
                                                    <span className={`text-xs font-bold px-2 py-0.5 rounded border uppercase tracking-wide ${hearing.bill_status === 'Passed' ? 'bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-300 border-green-200 dark:border-green-800' :
                                                        hearing.bill_status === 'Vetoed' ? 'bg-red-50 dark:bg-red-900/30 text-red-700 dark:text-red-300 border-red-200 dark:border-red-800' :
                                                            hearing.bill_status === 'Floor Vote' ? 'bg-yellow-50 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300 border-yellow-200 dark:border-yellow-800' :
                                                                'bg-gray-50 dark:bg-slate-700 text-gray-700 dark:text-slate-300 border-gray-200 dark:border-slate-600'
                                                        }`}>
                                                        {hearing.bill_status}
                                                    </span>
                                                )}
                                                <span className="text-xs font-bold text-black dark:text-slate-300 bg-granite-100 dark:bg-slate-700 uppercase tracking-wide border border-granite-400 dark:border-slate-500 px-2 py-0.5 rounded">
                                            ...
                                            {hearing.ai_summary && (
                                                <div className="bg-indigo-50/50 border border-indigo-100 rounded-lg p-3 mb-3">
                                                    <p className="text-sm text-granite-800 leading-relaxed">
                                                        <span className="font-bold text-indigo-900">Summary: </span>
                                                        {hearing.ai_summary}
                                                    </p>
                                                </div>
                                            )}

                                            <div className="text-granite-800 space-y-1 text-sm">
                                                <p className="flex items-center gap-2">
                                                    <span className="font-bold w-20 text-granite-900">Committee:</span>
                                                    {hearing.committee}
                                                </p>
                                                <p className="flex items-center gap-2">
                                                    <span className="font-bold w-20 text-granite-900">Location:</span>
                                                    {hearing.location}
                                                </p>
                                            </div>
                                        </div>

                                        <div className="flex flex-col items-center justify-center bg-granite-50 rounded-lg p-4 min-w-[120px] text-center border border-granite-200 self-start">
                                            <span className="text-sm font-bold text-black uppercase">
                                                {parseLocalDay(hearing.date).toLocaleDateString('en-US', { weekday: 'short' })}
                                            </span>
                                            <span className="text-2xl font-black text-granite-900">
                                                {parseLocalDay(hearing.date).getDate()}
                                            </span>
                                            <span className="text-sm font-bold text-granite-800">
                                                {hearing.time}
                                            </span>
                                        </div>
                                    </div>

                                    <div className="mt-6 pt-4 border-t border-granite-50 flex items-center justify-end gap-3">
                                        <Link
                                            href={`/${stateCode.toLowerCase()}/wizard?bill=${hearing.bill_number}&committee=${encodeURIComponent(hearing.committee)}&date=${hearing.date}` +
                                                (hearing.sponsor_info ? `&sponsor=${encodeURIComponent(hearing.sponsor_info.name)}&party=${hearing.sponsor_info.party}&election=${hearing.sponsor_info.next_election}` : '')}
                            ...
                            {savedHearings.length > 0 && (
                                <div className="space-y-4">
                                    ...
                                    {savedHearings.map(hearing => {
                                        // Reuse Hearing Card Logic (simplified for brevity or duplication?)
                                        // Ideally we extract HearingCard component. For now, duplication with Bookmark support
                                        // I'll reuse the exact structure to keep it consistent
                                        const smartPills = hearing.smart_pills || [];
                                        return (
                            ...
                            <div className="flex items-start justify-between gap-4">
                                <div className="flex-1">
                                    ...
                                    <div className="bg-orange-50/50 border border-orange-100 p-3 rounded-lg">
                                        <p className="text-sm font-bold text-orange-900 mb-1">Action Taken:</p>
                                        <p className="text-sm text-granite-800 dark:text-slate-300 mb-2">{bill.amendment_action}</p>
                                        <p className="text-xs text-granite-500 dark:text-slate-400 font-mono">{new Date(bill.amendment_date).toLocaleDateString()}</p>
                                    </div>
                                </div>
